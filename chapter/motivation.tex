...
Interfaces can not only be categorized by their intended demographics. Input- and output-devices dictate the kind of interaction. As mentioned earlier, keyboard and mouse are being replaced with novel technologies. Touchscreens and voice recognition have become an established device as well. Moreover, all kinds of novel approaches towards interaction are made and demanded for novel possibilities in technology.
\\
\textbf{Eingabemethoden - Novel Interaction Techniques}
\begin{itemize}
	\item Tangible interaction~\cite{TangibleUI}
	\item VR-interaction~\cite{VRObjectSelectionCnG}
	\item Interactive Museum Exhibits~\cite{InteractiveMuseumExhibits}
\end{itemize}
