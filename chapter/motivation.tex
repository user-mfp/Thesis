...
\begin{figure}[H]%
\includegraphics[width=\columnwidth]{../pics/pda-guide.eps}%
\caption{A conceptual sketch of the guide system (left) and the GUI of its application on a PDA~\cite{MuseumGuideSystems}.}%
...
\begin{figure}[H]%
\includegraphics[width=\columnwidth]{../pics/rhino.eps}%
\caption{An overview of the RHINO tour guide robot~\cite{RHINO}.}%
\label{fig:rhino} %
\end{figure}

In 2002, a group from the \textit{University of Limmerick} made a survey in \textit{Hunt Museum}. The museum is owned and run by the Hunt-family. Its tradition is to involve the visitors since its early days. Therefor, they had so-called \textit{cabinets of curiosity}~\cite{Hunt1}, special compartments within the exhibition, where additional exhibits were hidden. For example, a curious visitor had to open drawers in order to find a collection of plates (see Figure \ref{fig:hunt_museum}). Via this exploration, the visitors became involved. Inspired by their observations, Ciolfi et al. implemented a completely new and interactive part of the exhibition in 2003~\cite{Hunt2}. Two new rooms were introduced. First, there was the \textit{study room} with three interactive devices for getting further information about certain exhibits. They were disguised as a chest, a painting and the interactive desk depicted in Figure \ref{fig:hunt_museum}. The second room, the \textit{room of opinion}, was plain white with plinths, on which visitors could record their interpretations of the intended function of certain exhibits. In  order to manage all the data, a third and hidden room was used to host all the data-servers.
\begin{figure}[H]%
\includegraphics[width=\columnwidth]{../pics/hunt.eps}%
\caption{Cabinets of curiosity at the Hunt Museum (left)~\cite{Hunt1} and a conceptual drawing (middle) [ibid.] of the later interactive desk (right)~\cite{Hunt2}.}%
...
\begin{figure}[H]%
\includegraphics[width=\columnwidth]{../pics/medien-welten.eps}%
\caption{Abacus, news-studio, and telegraph installation at the medien.welten~\cite{MedienWelten}.}%
\label{fig:medien_welten} %
\end{figure}
Visitors of medien.welten could not only use the devices, but also store some of their produced contents in a \textit{digital backpack} [ibid.]. This way, visitors did not only have an exciting experience, but also something to remember it by later.
...
\begin{figure}[H]%
\includegraphics[width=\columnwidth]{../pics/putthatthere.eps}%
\caption{Sketch of the Media Room (left) and interaction inside it with free-hand pointing gestures and voice commands~\cite{PutThatThereUI}.}%
...
\begin{figure}[H]%
\includegraphics[width=\columnwidth]{../pics/kinoogle.eps}%
\caption{Gestures used for panning (a), zooming (b), rotating (c), and tilting (d) maps in Kinoogle~\cite{Kinoogle}.}%
\label{fig:kinoogle} %vire gesten zur steuerung
\end{figure}
...
\\
In virtual \ac{3D}-environments low cost depth sensors are used as well. Thus, a Kinect can be utilized to track a user inside a \ac{CAVE}\footnote{A \ac{CAVE} is a special \ac{VR}-setup with multiple screens. This produces more immersion than an ordinary single screen-setup. To provide correct perspectives for a user, its head position and orientation have to be reliably tracked. Otherwise, there might be unpleasant artifacts or even errors between the different screens.} to calculate correct perspectives for him or her~\cite{KinectCAVE}. Figure \ref{fig:kinectCAVE} shows a user inside the \ac{CAVE} once as a standard image and once as depth image.

\begin{figure}[H]%
\includegraphics[width=\columnwidth]{../pics/cave.eps}%
\caption{User inside the CAVE at HTW Berlin seen with in a normal image (left) and by the depth sensor with the joints of the skeleton tracking (right)~\cite{KinectCAVE}.}%
