...
The characteristic functionality of the Exhibition-class is provided by its get- and set-methods. They allow for reading and writing all those essential variables.
...
\begin{figure}[H]%
\includegraphics[width=\columnwidth]{../pics/er-diagram.eps}%
...
\begin{figure}[H]%
\includegraphics[width=\columnwidth]{../pics/pedalpoint.eps}%
...
\begin{figure}[H]%
\includegraphics[width=\columnwidth]{../pics/intersection.eps}%
...
\\
A position on an \ac{IMI}-exhibit has to be validated as well. Hence, the defining processes is repeated and the two centers are compared according to the threshold of the respective \ac{IMI}-exhibition. If they validate each other, the position is saved for the particular \ac{IMI}-exhibit. Should corners or a position not successfully validate, the whole definition process has to be repeated.
\\
...
\begin{figure}[H]%
\includegraphics[width=\columnwidth]{../pics/kernels.eps}%
\caption{The basic principle of a kernels. Two targets with equal kernels (left) and two targets with a big and small AOA (right).}%
\label{fig:exhibit_kernel} %a) basic principle and b) comparison of big an small AOA 
\end{figure}
\ac{AOA}s can overlap each other when \ac{IMI}-exhibits are very close to each other. In this case, a more dominant kernel with bigger size and radius increases the \ac{AOA} of the respective \ac{IMI}-exhibit. Hence, the wider \ac{AOA} attracts more selections than the one of its neighbor.
